...
export default function EditPatient() {
    ...
    return (
      <div className="relative" ref={dropdownRef}>
        ...
        <button
          ...
          className="w-full border border-gray-300 rounded-lg p-2 text-left bg-white hover:border-gray-400 flex justify-between items-center"
        >
    ...
    const fetchPatient = async () => {
      try {
        ...
        if (data.followUp?.date)
          data.followUp.date = new Date(data.followUp.date)
            .toISOString()
            .split("T")[0];
  ...
  const removeDiagnosis = (index) => {
    const updated = form.initialDiagnosis.filter((_, i) => i !== index);
    setForm({ ...form, initialDiagnosis: updated });
  };
  ...
  if (loading) return <div className="pt-20 p-6">กำลังโหลดข้อมูล...</div>;
      ...
      >
        ...
        <div className="grid grid-row-2 gap-4">
          <Input
            ...
            onChange={(e) => handleChange("diffSymptoms", e.target.value)}
          />
          <Input
            label="โรคทั้งหมด"
            value={form.diffDiseases}
            onChange={(e) => handleChange("diffDiseases", e.target.value)}
          />
        </div>
